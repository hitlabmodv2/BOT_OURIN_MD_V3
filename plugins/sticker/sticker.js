...
import { replyHydro } from '../../src/lib/ourin-context.js'
...
async function handler(m, { sock, config: botConfig, skipDeduct }) {
    const isImage = m.isImage || (m.quoted && m.quoted.type === 'imageMessage')
    const isVideo = m.isVideo || (m.quoted && m.quoted.type === 'videoMessage')
    
    if (!isImage && !isVideo) {
        skipDeduct?.()
        await replyHydro(sock, m,
            `🖼️ *sᴛɪᴄᴋᴇʀ ᴍᴀᴋᴇʀ*\n\n` +
    ...
    try {
        ...
        if (!buffer) {
            await replyHydro(sock, m, '❌ Gagal mendownload media!')
            await m.react('❌')
            return
        }
        ...
        if (isVideo) {
            ...
            try {
                ...
                if (duration > 10) {
                    await replyHydro(sock, m, `❌ Video terlalu panjang!\n\n> Durasi: ${duration.toFixed(1)} detik\n> Maksimal: 10 detik`)
                    await m.react('☢')
