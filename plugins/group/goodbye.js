  ...
  try {
    ...
    try {
      ...
      if (r.data && r.data.byteLength > 500) {
        ...
        console.log(`[Goodbye] PP user berhasil: ${r.data.byteLength} bytes`);
      }
    } catch (e) {
      console.log(`[Goodbye] PP user gagal: ${e.message}`);
    }
    ...
    if (!ppBuffer) {
      try {
        if (fs.existsSync(ppKosongPath)) {
          ...
          console.log(`[Goodbye] Fallback pp-kosong.jpg: ${ppBuffer.byteLength} bytes`);
        }
      } catch { }
    }
    console.log(`[Goodbye] ppBuffer status: ${ppBuffer ? `ada (${ppBuffer.byteLength} bytes)` : "NULL - thumbnail akan kosong"}`);
