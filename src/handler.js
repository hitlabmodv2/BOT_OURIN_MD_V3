...
import { logger, logMessage, c, chalk } from "./lib/ourin-logger.js";
import {
  isLid,
  isLidConverted,
  lidToJid,
...
async function messageHandler(msg, sock, options = {}) {
  ...
  try {
    ...
    if (config.features?.logMessage && m.fromMe && !isJadibot) {
    ...
    // Simpan info deduct — akan diterapkan SETELAH command berhasil
    let _shouldDeductEnergi = false;
    let _energiAmount = 0;
    let _energiSisa = 0;
    ...
    if (energiEnabled && plugin.config.energi > 0) {
      ...
      if (
        ...
        // owner/partner unlimited — tidak perlu deduct
      } else if (m.isPremium && (premiumEnergi === -1 || user?.energi === -1)) {
        // premium unlimited — tidak perlu deduct
      } else {
        currentEnergi =
          user?.energi ??
          (m.isOwner || m.isPartner
        ...
        if (currentEnergi < plugin.config.energi) {
          await m.reply(getMsg("energiExceeded") || "⚡ Energi habis!");
          return;
        }
        // Tandai untuk deduct setelah command sukses
        _shouldDeductEnergi = true;
        _energiAmount = plugin.config.energi;
        _energiSisa = currentEnergi - plugin.config.energi;
      }
    }
    ...
    try {
      const _time = timeHelper.formatTime("HH:mm:ss");
      const _cmdPrefix = m.prefix || config.command?.prefix || ".";
      const _cmdText = m.text ? ` ${m.text.substring(0, 35)}` : "";
      const _senderName = m.pushName || m.sender?.split("@")[0] || "?";
      const _senderNum = m.sender?.split("@")[0] || "";
      if (m.isGroup) {
        const _gd = db.getGroup(m.chat);
        let _gname = _gd?.name || "";
        if (!_gname || _gname === "Unknown" || _gname === "Unknown Group") {
          sock.groupMetadata(m.chat).then((meta) => {
            if (meta?.subject) db.setGroup(m.chat, { name: meta.subject });
          }).catch(() => {});
          _gname = m.chat.replace("@g.us", "");
        }
        const _tag = chalk.gray("[") + " " + chalk.greenBright("GC") + " " + chalk.gray("]");
        console.log(`${_tag} ${chalk.gray(_time)} ${chalk.greenBright(_senderName)} ${chalk.gray("·")} ${chalk.green(_gname)} ${chalk.gray("›")} ${chalk.whiteBright(_cmdPrefix + m.command)}${chalk.gray(_cmdText)}`);
      } else {
        const _tag = chalk.gray("[") + " " + chalk.cyanBright("PM") + " " + chalk.gray("]");
        console.log(`${_tag} ${chalk.gray(_time)} ${chalk.cyanBright(_senderName)} ${chalk.gray("·")} ${chalk.cyan(_senderNum)} ${chalk.gray("›")} ${chalk.whiteBright(_cmdPrefix + m.command)}${chalk.gray(_cmdText)}`);
      }
    } catch {}
    ...
    if (_shouldDeductEnergi) {
      db.updateEnergi(m.sender, -_energiAmount);
      if (db.setting("notiflimit")) {
        let limitMsg = getMsg("limitDeducted") || "🔋 Limit kau berkurang sebanyak {amount}. Sisa limit: {sisa}";
        limitMsg = limitMsg.replace("{amount}", _energiAmount.toString()).replace("{sisa}", _energiSisa.toString());
