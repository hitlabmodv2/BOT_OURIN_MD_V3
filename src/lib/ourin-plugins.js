...
function printPluginTable(plugins) {
  ...
  const row = (content) => {
    ...
    return `  ${chalk.gray("│")} ${content}${pad} ${chalk.gray("│")}`;
  };

  const hdr = `plugins · ${plugins.length} · ${catCount} kat`;
  const fillTotal = IW + 2;
  const decorated = `─ ${hdr} ─`;
  let topBorder;
  if (decorated.length <= fillTotal) {
    const sL = Math.floor((fillTotal - decorated.length) / 2);
    const sR = fillTotal - sL - decorated.length;
    topBorder = `  ${chalk.gray("╭" + "─".repeat(sL) + "─ " + hdr + " ─" + "─".repeat(sR) + "╮")}`;
  } else {
    topBorder = `  ${chalk.gray("╭" + "─".repeat(fillTotal) + "╮")}`;
  }

  const padEntry = (cat, count) =>
    `${chalk.whiteBright(String(cat).padEnd(7))}${chalk.greenBright(String(count).padStart(4))}`;

  console.log("");
  console.log(topBorder);

  for (let i = 0; i < top.length; i += 2) {
    const left = padEntry(top[i][0], top[i][1]);
    const right = i + 1 < top.length
      ? `  ${padEntry(top[i + 1][0], top[i + 1][1])}`
      : " ".repeat(13);
    console.log(row(left + right));
  }

  if (rest.length > 0) {
    const restLine = `${chalk.gray("+" + rest.length + " lainnya")}  ${chalk.gray(String(restTotal))}`;
    console.log(row(restLine));
  }

  console.log(`  ${chalk.gray("╰" + "─".repeat(fillTotal) + "╯")}`);
  console.log("");
}
