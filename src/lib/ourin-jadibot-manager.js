...
function getJadibotAuthPath(jid) {
  ...
  return path.join(JADIBOT_AUTH_FOLDER, id + ".json");
}
...
function getAllJadibotSessions() {
  ...
  const files = fs.readdirSync(JADIBOT_AUTH_FOLDER);
  for (const file of files) {
    if (!file.endsWith(".json")) continue;
    const id = file.slice(0, -5); // hapus .json
    const sessionFile = path.join(JADIBOT_AUTH_FOLDER, file);
    sessions.push({
      id,
      jid: id + "@s.whatsapp.net",
      isActive: jadibotSessions.has(id),
      sessionFile,
    });
  }
  return sessions;
...
async function startJadibot(sock, m, userJid, usePairing = true) {
  ...
  const { state, saveCreds } = await useSingleFileAuthState(authPath);
