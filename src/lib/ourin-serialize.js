...
import { saluranCtx, sendReplyVariant } from "./ourin-context.js";
...
function getMessageBody(message, type) {
  ...
  switch (type) {
    ...
    case "templateButtonReplyMessage":
      return messageContent.selectedId || messageContent.selectedDisplayText || "";
    case "interactiveResponseMessage":
      try {
        ...
        if (messageContent.body?.text) return messageContent.body.text;
        return "";
      } catch {
        return "";
      }
...
async function serialize(sock, msg, store = {}) {
  ...
  m.reply = async (text, options = {}) => {
    return sendReplyVariant(sock, m, msg, text, options);
  };
