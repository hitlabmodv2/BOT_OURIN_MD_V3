...
export function getMsg(key) {
  ...
  return db.setting(`customMsg_${key}`) || config.messages?.[key] || null;
}
...
function checkPermission(m, pluginConfig) {
  ...
  if (pluginConfig.isOwner && !m.isOwner && !hasAccess) {
    return {
      allowed: false,
      reason: getMsg("ownerOnly") || "🚫 Owner only!",
    };
  }

  if (pluginConfig.isPartner && !m.isPartner && !m.isOwner && !hasAccess) {
    return { allowed: false, reason: getMsg("partnerOnly") || "🤝 Partner only!" };
  }
  ...
  if (
    ...
    return {
      allowed: false,
      reason: getMsg("premiumOnly") || "💎 Premium only!",
    };
  }
  ...
  if (pluginConfig.isGroup && !m.isGroup) {
    return {
      allowed: false,
      reason: getMsg("groupOnly") || "👥 Group only!",
    };
  }
  ...
  if (pluginConfig.isPrivate && m.isGroup) {
    return {
      allowed: false,
      reason: getMsg("privateOnly") || "📱 Private chat only!",
    };
  }
  ...
  if (
    ...
    return {
      allowed: false,
      reason: getMsg("adminOnly") || "👮 Admin grup only!",
    };
  }
  ...
  if (pluginConfig.isBotAdmin && m.isGroup && !m.isBotAdmin) {
    return {
      allowed: false,
      reason: getMsg("botAdminOnly") || "🤖 Bot harus menjadi admin grup!",
    };
  }
