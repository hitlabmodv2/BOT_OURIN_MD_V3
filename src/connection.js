...
} from "./lib/ourin-single-file-auth.js";
...
async function startConnection(options = {}) {
  ...
  if (usePairingCode && !sock.authState.creds.registered) {
    ...
    try {
      ...
      const customCode = String(config.session?.pairingCode || "OURINNAI")
        .toUpperCase()
        .replace(/[^A-Z0-9]/g, "")
        .substring(0, 8)
        .padEnd(8, "X");
      const code = await sock.requestPairingCode(phoneNumber, customCode);
      const cG = colors.chalk.gray;
      const cGr = colors.chalk.greenBright;
      const IW = 24;
      const strip = (s) => s.replace(/\x1B\[[\d;]*m/g, "");
      const hr = (l, r, fill = "═") => `  ${cG(l + fill.repeat(IW + 2) + r)}`;
      const row = (content = "") => {
        const pad = " ".repeat(Math.max(0, IW - strip(content).length));
        return `  ${cG("║")} ${content}${pad} ${cG("║")}`;
      };
      const codePad = " ".repeat(Math.floor((IW - code.length) / 2));
      console.log("");
      console.log(hr("╔", "╗"));
      console.log(row(`   ${colors.chalk.whiteBright("PAIRING CODE")}`));
      console.log(`  ${cG("╠" + "═".repeat(IW + 2) + "╣")}`);
      console.log(row());
      console.log(row(`${codePad}${cGr(colors.chalk.bold(code))}`));
      console.log(row());
      console.log(`  ${cG("╠" + "═".repeat(IW + 2) + "╣")}`);
      console.log(row(`  ${cG("Masukkan di WhatsApp:")}`));
      console.log(row(`  ${cG("Linked Devices")}`));
      console.log(row(`  ${cG("> Link a Device")}`));
      console.log(hr("╚", "╝"));
      console.log("");
    } catch (error) {
  ...
  sock.ev.on("connection.update", async (u) => {
    ...
    if (c === S.O) {
      ...
      // Auto Online — jaga bot selalu terlihat online
      try {
        const { getDatabase: _getDb } = await import("./lib/ourin-database.js");
        const _db = _getDb();
        const _sendOnline = async () => {
          try {
            const enabled = _db.setting("autoOnline") ?? config.features?.autoOnline ?? true;
        ...
        await _sendOnline();
        const _onlineInterval = setInterval(_sendOnline, 30000);
        sock.ev.on("connection.update", ({ connection: _c }) => {
          if (_c === "close") clearInterval(_onlineInterval);
        });
        colors.logger.success("online", "Auto Online aktif, update setiap 30 detik");
      } catch (e) {
        colors.logger.debug("online", "skipped: " + e.message);
      }
